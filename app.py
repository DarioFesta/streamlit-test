...
uploaded_file = st.file_uploader(label = "Choose a file/files in csv or bin format - Don't mix csv and bin files",
                                    accept_multiple_files = True, type = ["csv", "bin"])
...
def load_multiple_bin_files_horizontally(uploaded_bin_files):
    ...
    for file_bin in uploaded_bin_files:
        file_bin.seek(0)
        # st.file_uploader() returns a BytesIO object,which is a buffer,
        # to get the bytes from a BytesIO object, I need to read the buffer first
        ...
        df['timestamp'] = df['time(s)'] + (df['time(ns)'] / 1000000000)
        df.drop(['time(s)', 'time(ns)'], inplace = True, axis = 1)
        df[param_name] = df[param_name].astype("float32") # save memory space (don't need to use float64 for parameters)
        l.append(df)
    combined_bin_files = pd.concat(l, axis = 1)
    # eliminate the duplicate columns with the timestamp info
    combined_bin_files = combined_bin_files.loc[:, ~combined_bin_files.columns.duplicated()]
    return combined_bin_files

@st.cache(allow_output_mutation=True)
def load_multiple_bin_files_vertically(uploaded_bin_files):
    l = []
    for file_bin in uploaded_bin_files:
        file_bin.seek(0)
        # st.file_uploader() returns a BytesIO object,which is a buffer,
        ...
        bytes_data = file_bin.read()
        param_name = file_bin.name[:-4] # excldue '.bin' from the name
        # IId =   LONG (4 bytes for time in seconds) + LONG(4 bytes for nanoseconds) + DOUBLE (8 bytes for parameter value)
        dt = np.dtype( [('time(s)', 'I'), ('time(ns)', 'I'), (param_name, 'd') ])
        #offset = 96 because the first 6 bytes are metadata,as per GETPKT-INST-001.
        np_file = np.frombuffer(bytes_data, dtype = dt, offset = 96)
        # convert to dataframe
        ...
        df['timestamp'] = df['time(s)'] + (df['time(ns)'] / 1000000000)
        df.drop(['time(s)', 'time(ns)'], inplace = True, axis = 1)
        df[param_name] = df[param_name].astype("float32")
        l.append(df)
    combined_bin_files = pd.concat(l, axis = 0)
    # eliminate the duplicate columns with the timestamp info
    #combined_bin_files = combined_bin_files.loc[:, ~combined_bin_files.columns.duplicated()]
    return combined_bin_files
...
def display_plots_no_time(df_csv, parameters):
    ...
    for param in parameters:
        fig.add_trace(go.Scatter(y = df_csv[param].astype('float32'), name=param))
...
def display_plots_with_time(df_csv, parameters, column_name_time = None):
    ...
    fig = go.Figure()
    for param in parameters:
        fig.add_trace(go.Scatter(x = df_csv[column_name_time], y = df_csv[param].astype('float32'), name=param))
    fig.update_layout(legend=dict(yanchor="top", y=1.2, xanchor="center", x=0.5, orientation="h"),
            plot_bgcolor='rgb(255, 255, 251)',                #title_text='Plot'
            xaxis=dict(
            linecolor="#BCCCDC",
...
def display_subplots_no_time(df_csv, parameters):
    ...
    for param in parameters:
        fig.add_trace(go.Scatter(y = df_csv[param].astype('float32'), name=param), row=parameters.index(param) +1, col=1)
...
if uploaded_file:
    ...
    if len(uploaded_file) > 1:
        ...
        if choice_concat == "Horizontally":
            try:
                file = load_multiple_csv_files_horizontally(uploaded_file)
                list_of_parameters, time_option, column_name_time = data_info(file)
            except:
                file = load_multiple_bin_files_horizontally(uploaded_file)
                list_of_parameters, time_option, column_name_time = data_info(file)


        if choice_concat == "Vertically":
            try:
                file = load_multiple_csv_files_vertically(uploaded_file)
                list_of_parameters, time_option, column_name_time = data_info(file)
            except:
                file = load_multiple_bin_files_vertically(uploaded_file)
                list_of_parameters, time_option, column_name_time = data_info(file)
...
# st.markdown(
#     """
# <style>.css-1aumxhk {background-color: #d4f0e0; background-image: none; color: #ffff00}
# </style>
# """,
